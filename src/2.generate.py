...
with open(f'./input/data_part_{volume}.json', 'r') as f:
    data = json.load(f)
...
for document in tqdm(unprocessed_data):
    try:
        print(document['_id'])
        model4_new = SG(r=document['r'], n_t=document['n_t'], cr=0, T_re=document['T_re'], DN_re=document['DN_re'], kappa10=document['kappa10'],
                        Omega_bh2=document['Omega_bh2'], Omega_ch2=document['Omega_ch2'], H0=document['H0'], A_s=document['A_s'])
